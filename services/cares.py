...
def create_care(care: CaresCreate, db):
    ...
    doctor_patient = db.query(DoctorPatients).filter(DoctorPatients.c.doctor_id == care.doctor_id,  DoctorPatients.c.patient_id == care.patient_id).first()

    if doctor_patient is None:
        return None
    care_bd = Cares(description=care.description, doctor_patients_id=doctor_patient.id)
    db.add(care_bd)
    db.commit()
    db.refresh(care_bd)
    return care_bd
...
def get_cares_by_patient_id(id:int, db):
    cares_bd: Cares = db.query(Cares).options(joinedload(Cares.doctor).joinedload(Doctor.user)).join(DoctorPatients, Cares.doctor_patients_id == DoctorPatients.c.id).filter(DoctorPatients.c.patient_id == id).all()
