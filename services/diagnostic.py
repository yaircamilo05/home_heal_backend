...
def create_diagnostic(diagnostic: DiagnosticCreate, db):
    ...
    if doctor_patient is None:
        return None
    diagnostic_bd = Diagnostic(
        description=diagnostic.description,
        doctor_patients_id=doctor_patient.id, patient_id=diagnostic.patient_id
    )
...
def get_diagnostic_by_patient_id(id: int, db):
    diagnostics_bd: Diagnostic = db.query(Diagnostic).options(joinedload(Diagnostic.doctor).joinedload(Doctor.user)).join(
        DoctorPatients, Diagnostic.doctor_patients_id == DoctorPatients.c.id).filter(DoctorPatients.c.patient_id == id).all()
